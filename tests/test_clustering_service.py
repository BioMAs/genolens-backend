...
# ─────────────────────────────────────────────────────────────────────────────
# CL-01 to CL-04 — edge-case complements (from FIXPLAN.md §3.1)
# ─────────────────────────────────────────────────────────────────────────────

class TestClusteringEdgeCases:
    """Complement tests targeting edge cases identified in FIXPLAN.md."""

    # CL-01: DataFrame with NaN → imputation, no crash
    def test_cl01_nan_values_imputed_without_crash(self):
        """NaN values are imputed (e.g. with 0) so clustering does not crash."""
        service = _make_service()
        df = _make_df(n_genes=15, n_samples=6)
        df.iloc[2, 3] = float("nan")
        df.iloc[7, 1] = float("nan")

        result = service.perform_clustering(df, cluster_rows=True, cluster_cols=True)
        assert "row_order" in result
        assert len(result["row_order"]) == 15

    # CL-02: k > number of genes → clamped
    def test_cl02_kmeans_k_greater_than_n_genes_clamped(self):
        """k-means with k > n_genes should be clamped and not crash."""
        service = _make_service()
        n_genes = 5
        df = _make_df(n_genes=n_genes, n_samples=6)

        # k=100 >> n_genes=5 → should be clamped internally
        result = service.perform_clustering(
            df, cluster_rows=True, cluster_cols=False, method="kmeans", n_clusters=100
        )
        assert "row_order" in result
        assert len(result["row_order"]) == n_genes

    # CL-03: ward + euclidean on 5 000 genes < 10 s
    @pytest.mark.slow
    def test_cl03_performance_5000_genes(self):
        """Hierarchical ward clustering on 5 000 genes × 20 samples finishes < 10 s."""
        import time
        service = _make_service()
        df = _make_df(n_genes=5_000, n_samples=20, seed=7)
    ...
    def test_cl04_nonexistent_gene_ids_raises_value_error(self):
        """Requesting gene_ids that don't exist in the dataset raises a clear error."""
        service = _make_service()
        df = _make_df(n_genes=20, n_samples=6)

        with pytest.raises((ValueError, KeyError, Exception)):
            service.perform_clustering(
                df, gene_ids=["PHANTOM_GENE_1", "PHANTOM_GENE_2"]
            )
